import * as skinview3d from "../src/skin3d";
import "./style.css";
...
(async function () {
	...
	for (const config of configurations) {
		await Promise.all([
			...
			// skinViewer.loadCape(config.cape ?? null, { backEquipment: config.backEquipment }),
		]);
		skinViewer.render();
		...
		const renderedImgs = document.getElementById("rendered_imgs");
		if (renderedImgs) {
			renderedImgs.appendChild(imgElement);
		}
	}
