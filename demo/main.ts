...
let skinViewer: skin3d.View;

function getTextureURL(id: string): string {
...
function reloadSkin(): void {
	...
	const url = getTextureURL("skin_url");
	if (url === "") {
...
function reloadCape(): void {
	...
	const url = getTextureURL("cape_url");
	if (url === "") {
...
function reloadEars(skipSkinReload = false): void {
	...
	if (sourceType === "none") {
		...
		const url = getTextureURL("ears_url");
		if (url === "") {
			skinViewer.loadEars(null);
			input?.setCustomValidity("");
		} else {
...
function reloadPanorama(): void {
	...
	const url = getTextureURL("panorama_url");
	if (url === "") {
...
function initLiveControls(): void {
	...
	for (const el of animationRadios) {
		el.addEventListener("change", e => {
			...
			if (crouchSetting) {
				crouchSetting.style.display = animationCrouch?.checked ? "flex" : "none";
			}
...
function initView(): void {
	...
	skinViewer.width = Number(500);
	skinViewer.height = Number(450);
